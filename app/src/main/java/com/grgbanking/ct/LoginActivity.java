...
//						}
		            	if(!ResultInfo.CODE_SUCCESS.equals(resultInfo.getCode())){
							...
							Log.v("message:",resultInfo.getMessage());
...
//						if (!loginNameViewValue.equals("cfl")) {
//							Toast.makeText(context, "用户名或密码错误，请重新再输。", 5000).show();
//		            		loginButtonView.setText("登录");
//		            		loginButtonView.setEnabled(true);
//		            		return;
//						}
//						
//						if (!passwordViewValue.equals("123456")) {
//							Toast.makeText(context, "用户名或密码错误，请重新再输。", 5000).show();
//		            		loginButtonView.setText("登录");
//		            		loginButtonView.setEnabled(true);
//		            		return;
//						}
		            	...
		            	//如果有勾记住用户名、密码，则将登录的信息写入手机本地缓存中
//		            	if(remPasswordView.isChecked()){
//		            		person.setSelected("1");
//		            	}else{
//		            		person.setSelected("0");
//		            	}
//		            	person.setLogin_name(loginNameViewValue);
//	            		person.setPassword(passwordViewValue);
//	            		person.setUser_name(userName);
//	            		person.setUser_id(userId);
//	            		if(person.getTableid()!=0){
//	            			PersonTableHelper.updateEntity(context, person);
//	            		}else{
//	            			PersonTableHelper.savePerson(context, person);
//	            		}
...
//		}
	...
	private void success(){
		...
		//	                    mBundle.putSerializable("pdaLoginMessage", pdaLoginMessage); // 传递一个user对象
